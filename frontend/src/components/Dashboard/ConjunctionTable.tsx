...
type SortField = 'risk_score' | 'miss_distance_km' | 'tca';
...
export default function ConjunctionTable() {
  ...
  const sorted = [...conjunctions].sort((a, b) => {
    ...
    switch (sortField) {
      ...
      case 'miss_distance_km':
        cmp = a.miss_distance_km - b.miss_distance_km;
        break;
      case 'tca':
        cmp = new Date(a.tca).getTime() - new Date(b.tca).getTime();
        break;
    }
  ...
  const handleRowClick = (conj: (typeof conjunctions)[0]) => {
        ...
        <table className="conjunction-table">
          <thead>
            <tr>
              ...
              >
                TCA <ArrowUpDown size={10} />
              </th>
              <th
                onClick={() => handleSort('miss_distance_km')}
                style={{ cursor: 'pointer' }}
              >
                Distance <ArrowUpDown size={10} />
              </th>
              <th
